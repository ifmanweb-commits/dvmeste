...
interface ReferencesClientProps {
  initialData: {
    'work-formats': string[];
    'paradigms': string[];
    'certification-levels': string[];
    'article-tags': string[];
  };
}

export default function ReferencesClient({ initialData }: ReferencesClientProps) {
  const [activeTab, setActiveTab] = useState<'work-formats' | 'paradigms' | 'certification-levels' | 'article-tags'>('work-formats');
  const [items, setItems] = useState<string[]>(initialData[activeTab]);
  ...
  const loadData = async () => {
    ...
    try {
      // Используем initialData вместо запроса к API
      const data = initialData[activeTab];
      console.log(`📊 Данные для ${activeTab}:`, data);
      setItems(data || []);
    } catch (error) {
  ...
  const handleTabChange = (tab: typeof activeTab) => {
    setActiveTab(tab);
    // Устанавливаем items из initialData
    setItems(initialData[tab]);
    setEditingIndex(null);
    setNewItem('');
    setError(null);
  };

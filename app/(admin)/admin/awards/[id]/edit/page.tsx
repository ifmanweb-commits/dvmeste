...
export default function EditAwardPage() {
  ...
  // Загрузка данных награды
  useEffect(() => {
    const fetchAward = async () => {
      try {
        const response = await fetch(`/api/admin/awards/${params.id}`);
        if (!response.ok) {
          throw new Error('Ошибка при загрузке награды');
        }
        const data = await response.json();
        setAward(data);
        setAwardType(data.type);
      } catch (err: any) {
  ...
  if (!award) {
    return (
      <div className="flex items-center justify-center min-h-screen">
        <div className="text-lg text-red-500">Награда не найдена</div>
      </div>
    );
  }
  ...
  return (
    <div>
      <div className="mb-6">
        <div className="flex justify-between items-center">
          <div>
            ...
            <h1 className="text-3xl font-bold text-gray-900 mt-2">Редактировать награду</h1>
          </div>
        </div>
        <HorNav />
      </div>
      ...
      <form onSubmit={handleSubmit} className="max-w-2xl space-y-6">
        ...
        {awardType === 'CERTIFICATE' && (
          <div>
            ...
            <select
              ...
              defaultValue={award.certificateTemplateId || ''}
              className="mt-1 block w-full rounded-lg border border-gray-300 px-3 py-2 focus:border-[#5858E2] focus:outline-none focus:ring-1 focus:ring-[#5858E2]"
            >
        ...
        {awardType === 'BADGE' && award.badgeUrl && (
          <div className="mb-4">
        ...
        {awardType === 'BADGE' && (
          <div>
            <label htmlFor="badge" className="block text-sm font-medium text-gray-700">
              Новый файл значка (оставьте пустым, чтобы не менять)
            </label>
            <input
              type="file"
              id="badge"

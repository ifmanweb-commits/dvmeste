...
import { updateCourseKey } from "@/lib/actions/courses";
import { useRouter } from "next/navigation";
import { useState } from "react";

interface EditCourseKeyFormProps {
  courseKeyId: string;
  courseId: string;
  initialData: {
    key: string;
...
export default function EditCourseKeyForm({ courseKeyId, courseId, initialData }: EditCourseKeyFormProps) {
  ...
  const handleSubmit = async (formData: FormData) => {
    ...
    const result = await updateCourseKey(courseKeyId, {
      key,
      status,
      maxUses,
      expiresAt,
    ...
    if (result.success) {
      ...
      setError(result.error || "Ошибка при обновлении промокода");
    }
  };

  const expiresAtValue = initialData.expiresAt
          ...
          <div className="mt-2 space-y-2">
            <label className="flex items-center">
              <input
                ...
                name="status"
                value="enrolled"
                defaultChecked={initialData.status === 'enrolled'}
                disabled={isSubmitting}
            ...
            <label className="flex items-center">
              <input
                ...
                name="status"
                value="graduated"
                defaultChecked={initialData.status === 'graduated'}
                disabled={isSubmitting}
                className="h-4 w-4 text-[#5858E2] focus:ring-[#5858E2]"
              />

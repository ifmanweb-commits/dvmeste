...
// Проверка: является ли ID draft-ключом
function isDraftKey(id: string): boolean {
  return id.startsWith("article-draft-");
}
...
export async function GET(
  ...
  try {
    ...
    // Если это draft-ключ — не проверяем БД, работаем только с ФС
    if (isDraftKey(id)) {
      ...
      try {
        ...
        console.log("No draft directory or error reading:", err);
      }
    ...
    // Проверяем существование статьи (для реальных ID)
...
export async function POST(
  ...
  try {
    ...
    // Если это draft-ключ — не проверяем БД, сохраняем только в ФС
    if (isDraftKey(id)) {
...
export async function DELETE(
  ...
  try {
    ...
    // Если это draft-ключ — удаляем только файл с диска (без БД)
    if (isDraftKey(id)) {
      if (!filename) {
        return NextResponse.json({ success: false, error: "Для draft-файлов требуется filename" }, { status: 400 });
      }
      
      const filePath = path.join(process.cwd(), "public", "files", "articles", id, filename);
